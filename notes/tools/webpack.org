#+TODO: TODO UNREAD UNWATCHED | READ WATCHED SKIPPED
...
** UNWATCHED Adding CSS to your build
** UNWATCHED Adding Images and Fonts to your build
** UNWATCHED Webpack Tools
...
***** Normal Chunks
Normal chunks don't contain Webpack runtime and can be loaded while the application is running via
a JSONP wrapper - aka lazy loading.
***** Initial Chunks
Normal chunks that count toward initial loading time of the app. These are generated by 
CommonsChunkPlugin. 
**** Lazy Loading with Webpack
We can defer loading of an asset until the user triggers some event using require.ensure. 
You can embed require.ensure in a promise wired to an event listener in your application code, and 
webpack will detect it statically, generate a separate bundle for the asset. For example, you could use 
require.ensure with routing; As a user enters some route, you begin to load the dependencies ahead of 
time.
**** Dynamic Loading with Webpack
require.context alows you to lazy load assets that aren't known at compile time. This could be useful for building tools, like a static site generator on top of webpack.
*** UNREAD Linting in Webpack
*** UNREAD Authoring Packages
*** UNREAD Writing Loaders
*** Configuring React
**** READ Setting up Babel with React
**** Setting up Hot Loading
**** UNREAD Using react-lite Instead of React for Production
**** UNREAD Exposing React Perf Utilities to 
**** UNREAD Optimizing Rebundling Speed During Development
**** UNREAD Setting Up Flow
**** UNREAD Setting Up Typescript
